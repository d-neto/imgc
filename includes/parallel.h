...
#define PARALLEL(__image_input, _slice_count, __operation) ({\
    img_t _image = (__image_input);\
    img_t _result = img_clone(_image);\
    ...
    if (_slice_count > _max_dim) {\
        ...
        img_t _slices[_slice_count];\
        img_t _outputs[_slice_count];\
        ...
        for (int i = 0; i < _slice_count; ++i) {\
            int start = i * _base + (i < _remainder ? i : _remainder);\
            if (_vertical) {\
                img_paste(_outputs[i], _result, 0, start);\
            } else {\
                img_paste(_outputs[i], _result, start, 0);\
            }\
        }\
    }\
    _result;\
